...
export class CliProgress {
    ...
    seenMessages = 0;
    startTime = Date.now();
    constructor(client, veContext, options) {
        this.client = client;
        ...
        while (Date.now() < deadline) {
            ...
            for (let i = this.seenMessages; i < messages.length; i++) {
                ...
                if (msg.finished) {
                    ...
                    if (!success) {
                        throw new ExecutionFailedError(`Execution failed at step '${msg.command}' (exit code ${msg.exitCode})`);
                    }
                    ...
                    if (msg.vmId !== undefined)
                        result.vmId = msg.vmId;
                    return result;
                }
            }
...
//# sourceMappingURL=cli-progress.mjs.map