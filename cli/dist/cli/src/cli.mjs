...
export class RemoteCli {
    ...
    async run() {
        ...
        const unresolvedResp = await this.client.getUnresolvedParameters(veContext, application, task);
        const parameterDefs = unresolvedResp.unresolvedParameters.filter((p) => !p.id.startsWith("addon_"));
        // 4. Resolve enum values
        const enumResp = await this.client.postEnumValues(veContext, application, task);
        ...
        let addons = [];
        try {
            const addonsResp = await this.client.getCompatibleAddons(application);
        ...
        try {
            ...
            const app = apps.find((a) => a.name === application || a.id === application);
        ...
        if (previousVmId) {
            try {
                const containerConfig = await this.client.getContainerConfig(veContext, Number(previousVmId.value));
        ...
        // 8. Validate
        const validationResult = await this.client.postValidateParameters(veContext, application, task, {
        ...
        // 9. Submit
        const configResp = await this.client.postVeConfiguration(veContext, application, task, {
            params: processedParams,
            ...(selectedAddons.length > 0 ? { selectedAddons } : {}),
            ...(disabledAddons.length > 0 ? { disabledAddons } : {}),
            ...(resolvedStackId ? { stackId: resolvedStackId } : {}),
    ...
    async resolveStack(requestedStackId, appStacktype, existingStacks) {
        ...
        if (requestedStackId) {
            ...
            if (!this.options.quiet) {
                process.stderr.write(`Stack '${requestedStackId}' not found. Creating stack '${requestedStackId}' (type: ${primaryStacktype})...\n`);
            }
        ...
        if (!this.options.quiet) {
            process.stderr.write(`No stacks found. Creating stack '${defaultName}' (type: ${primaryStacktype})...\n`);
        }
        await this.client.postCreateStack({
            name: defaultName,
            stacktype: primaryStacktype,
    ...
    readParametersFile(filePath) {
        ...
        if (!parsed.application || typeof parsed.application !== "string") {
            throw new CliError("Parameters file must contain an 'application' field", 1);
        }
        if (!parsed.task || typeof parsed.task !== "string") {
            throw new CliError("Parameters file must contain a 'task' field", 1);
        }
