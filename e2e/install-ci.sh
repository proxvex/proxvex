...
# Phase A1+A2 removed the runner→outer-host SSH path (qm via REST API,
# scp directly to nested VM). Phase B drops root inside the runner LXC.
# What's left for install-ci.sh: drop the OCI image, configure the LXC
# environment, set up the secrets bind-mount with the nested-VM key (and
# optionally the PVE API token + root CA so the runner can hit the API).
# No fresh keypair generation, no /root/.ssh seeding, no authorize-on-host.
...
configure_lxc() {
    ...
    local config_lines="lxc.init.cmd: /entrypoint-root.sh
...
configure_lxc "$RUNNER_HOST" "$RUNNER_VMID" \
    ...
    "PVE_HOST=$RUNNER_HOST" \
    "PVE_USE_API=1"

# Sanity check: warn if the API token / root CA secrets are not staged.
# Phase A1 needs both for the runner to hit the PVE REST API instead of SSH.
# These files are NOT generated by install-ci.sh — the operator drops them
# into $SECRETS_DIR_HOST manually after creating the API token via:
#   pveum user token add proxvex-runner@pam runner-token --privsep 1
ssh $SSH_OPTS "root@$RUNNER_HOST" "
    [ -s '$SECRETS_DIR_HOST/pve_api_token' ] || \
        echo 'WARN: $SECRETS_DIR_HOST/pve_api_token missing — runner will fall back to SSH for qm operations' >&2
    [ -s '$SECRETS_DIR_HOST/pve_root_ca.pem' ] || \
        echo 'WARN: $SECRETS_DIR_HOST/pve_root_ca.pem missing — TLS verification will be disabled if API path is forced' >&2
" || true
...
echo ""
echo "Container uses DHCP on $BRIDGE. The runner process runs as the unprivileged"
echo "'runner' user (Phase B); qm operations go through the PVE REST API"
echo "(Phase A1) using the token at \$SECRETS_DIR_HOST/pve_api_token."
echo ""
...
echo "  entrypoint-root.sh installs it as ~runner/.ssh/id_ed25519_nested at container start."
echo ""
echo "Operator-supplied secrets (drop into $SECRETS_DIR_HOST on $RUNNER_HOST):"
echo "  pve_api_token       Shell-sourceable: PVE_API_TOKEN_ID=...; PVE_API_TOKEN_SECRET=..."
echo "  pve_root_ca.pem     PEM-encoded PVE root CA (curl --cacert)."
echo "  Both must be chown'd to UID 100000 (LXC root) so entrypoint-root.sh can read them."
echo ""
