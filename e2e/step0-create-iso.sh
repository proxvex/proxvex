...
# Use config values. Per-instance paths so two parallel step0 runs for
# different instances don't clobber each other's work files on the PVE host
# or in the local /tmp directory.
WORK_DIR="/tmp/e2e-iso-build-${E2E_INSTANCE}"
LOCAL_ANSWER="/tmp/answer-e2e-${E2E_INSTANCE}.toml"
LOCAL_FIRSTBOOT="/tmp/first-boot-${E2E_INSTANCE}.sh"
...
pve_scp "$LOCAL_ANSWER" "root@$PVE_HOST:$WORK_DIR/answer-e2e.toml"
pve_scp "$HOST_SCRIPTS/create-iso.sh" "root@$PVE_HOST:$WORK_DIR/"
pve_scp "$LOCAL_FIRSTBOOT" "root@$PVE_HOST:$WORK_DIR/first-boot.sh"
