...
case $phase in
  post-start)
    ...
    systemd-run --no-block --unit="lxc-hook-${vmid}" pct exec "$vmid" -- sh -c "nohup /etc/lxc-oci-deployer/on_start_container ${APP_UID:-0} ${APP_GID:-0} >/etc/lxc-oci-deployer/hook.log 2>&1 </dev/null &"
    ;;
esac
